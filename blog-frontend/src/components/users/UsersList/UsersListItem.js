...
import { blockUser, unblockUser } from '../../../redux/slices/users/usersSlice';
...
const UsersListItem = (user) => {
  ...
  const sendEmailNavigate = () => {
    navigate('/send-email', {
      state: { email: user?.user?.email, id: user?.user?.id },
    });
  };
      ...
      <div className='p-8 mb-4 bg-white shadow rounded'>
        <div className='flex flex-wrap items-center -mx-4'>
          ...
          <div className='w-full flex lg:w-4/12 px-4  mb-6 lg:mb-0'>
            ...
            {user?.user?.isBlocked ? (
              <button
                onClick={() => dispatch(unblockUser(user?.user?.id))}
              ...
              <button
                onClick={() => dispatch(blockUser(user?.user?.id))}
